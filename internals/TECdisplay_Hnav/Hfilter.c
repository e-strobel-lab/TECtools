...
void Hfilter(constraint_metadata * cons_meta, char * prev_out_prfx, int cl, int layr_cnt, char * prev_TECDnav_out, char * layr_list[MAX_LAYERS])
{
    ...
    char cl_dir[256] = {0}; //current layer directory
    
    char TECDnav_command[MAX_LINE] = {0};  //TECdisplay_navigator command
    char TECDnav_out_dir[MAX_LINE] = {0};  //TECdisplay_navigator output directory name
    char ipt_data_file[MAX_LINE] = {0};    //TECdisplay_navigator input data file name
    char crnt_out_prfx[MAX_LINE] = {0};    //output file prefix for current TECdisplay_navigator run
    char *prfx2use = NULL;                 //pointer to output file prefix to be used for TECdisplay_navigator run
    char crnt_TECDnav_out[MAX_LINE] = {0}; //path to current TECdisplay_navigator output directory, passed to next Hfilter call

    int  prev_out_files = 0;               //number of output files generated by the previous layer
    
    char out_msg_dir[MAX_LINE] = {0};             //directory for storing output message files
    ...
    if (cl == 0 || cons_meta[cl-1].typ == 'x') { //if at layer 0 or if previous constraint excluded matches
    ...
    if (cl < layr_cnt) { //if the bottom layer has not yet been processed
        ...
        for (i = 0; i < prev_out_files; i++) { //for all previous output files
            ...
            if (cl == 0) { //if processing the first layer
                ...
                sprintf(ipt_data_file, "%s", TDHN_merged_out_nm);  //set the input data file to the merged input file
                ...
                for (j = 1; j <=cl ; j++) {            //append the remaining layer(s) file info to TECDnav_out_dir
                    ...
                    sprintf(TECDnav_out_dir, "%s_%s",  tmp_str, layr_list[j]);
                }
            ...
            strcpy(tmp_str, TECDnav_command);
            sprintf(TECDnav_command, "%s > %s/%s_out_msg.txt", tmp_str, out_msg_dir_name, TECDnav_out_dir);
