...
void Hfilter(constraint_metadata * cons_meta, char * prev_out_prfx, int cl, int layr_cnt, char * prev_TECDnav_out, char * layr_list[MAX_LAYERS])
{
    ...
    char ipt_data_file[MAX_LINE+1] = {0};    //TECdisplay_navigator input data file name
    ...
    if (cl < layr_cnt) { //if the bottom layer has not yet been processed
        ...
        for (i = 0; i < prev_out_files; i++) { //for all previous output files
            ...
            if (cl == 0) { //if processing the first layer
                strcpy(TECDnav_out_dir, cons_meta[cl].sn); //construct the TECDnav output directory name
                strcpy(ipt_data_file, TDHN_merged_out_nm); //set the input data file to the merged input file
                ...
                if ((snprintf(ipt_data_file, MAX_LINE, "%s/%s.txt", prev_TECDnav_out, crnt_out_prfx)) >= MAX_LINE) {
