...
void set_trgt(target * trgts, opt_mx_trg * trg_val, target * crnt_ref, char * trgt_id, char * trgt_sq)
{
    ...
    //TODO: double check that commented out code below is not needed (because validate_trgt_seq is run in parse_vmt_trgts and the return value of process_trgt_seq is always 1). are there other checks to do?
    ...
    if ((p_trg_val->vb_key = calloc(p_ref_val->vb_cnt+1, sizeof(*p_trg_val->vb_key))) == NULL) {
        printf("set_trgt: error - failed to allocate memory for variable base key. aborting...\n");
        abort();
    }
    
    get_key(p_trg_val->vb_key, trgts->sq, NULL, NULL, p_trg_val->ref, TARGET_KEY, MAX32CHAR_TARGET); //generate key
