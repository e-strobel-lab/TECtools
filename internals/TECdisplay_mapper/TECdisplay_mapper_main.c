...
int main(int argc, char *argv[])
{
    ...
    int run_mode = RUN_MODE_INIT;   //run mode setting
    ...
    while (1) {
        ...
        switch (c) {
            ...
            /* set standard flag */
            case 's':
                fastp_prms.mode = TDSPLY_STD;
                break;
                
            /* set barcoded flag */
            case 'b':
                fastp_prms.mode = TDSPLY_BRCD;
                break;
...
int check_options(int run_mode, int fq1_provided, int fq2_provided, int trgs_provided, fastp_params * fastp_prms, int trgt_ftype)
{
    ...
    if (fastp_prms->mode == TDSPLY_STD && trgt_ftype != VMT_FILE) {
    ...
    } else if (fastp_prms->mode == TDSPLY_BRCD && trgt_ftype != FASTA_FILE) {
