...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "m:i:I:t:o:p:dl:", long_options, &option_index);
    ...
    if (run_mode == MAP_SEQ_READS) {                                   //run MAP_SEQ_READS mode
        ...
        map_reads(&nm, fp_trgs, fastp_prms, &testdata, MAP_SEQ_READS); //map sequencing reads
        ...
        map_reads(&nm, fp_trgs, fastp_prms, &testdata, MAP_TEST_DATA   ); //map test data reads to target
