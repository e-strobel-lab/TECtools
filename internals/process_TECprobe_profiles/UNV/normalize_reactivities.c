...
void normalize_reactivities(SM2_analysis_directory * an_dir, int min_depth, double max_bkg, int norm_all, int verify_norm)
{
    ...
    for (i = 0, tot_rct_pf = 0; ix[i] <= an_dir->max_id; i++) {
        ...
        if (verify_norm) {
            ...
            cl_reactivity_list = calloc(an_dir->len[MAX]+1, sizeof(*reactivity_list)); //allocate rct list memory
        }

        //set pointer for tracking the number of reactivity
        //values from transcript lengths <100 and >=100
        if (an_dir->data[ix[i]].trg_nt_cnt < 100) {
        ...
        for (j = 0; j < an_dir->data[ix[i]].tot_nt_cnt; j++) {
            
            if (isHQnuc(&an_dir->data[ix[i]], j, min_depth, max_bkg)) {
                
                //reactivity value passed filter, copy to reactivity list
                //and increment tracking variable
                reactivity_list[tot_rct_pf++] = an_dir->data[ix[i]].reactivity_profile[j];
                (*p_tl100_tracking)++;
                
                //in verify_norm mode, add pf reactivity to list for the current target
                if (verify_norm) {
                    cl_reactivity_list[cl_rct_pf++] = an_dir->data[ix[i]].reactivity_profile[j];
                }
            }
        }
        ...
        if (verify_norm) {
            ...
            //calculate a normalization factor for the current target
            cl_norm_factor = calculate_normalization_factor(&cl_reactivity_list[0], cl_rct_pf, (an_dir->data[ix[i]].trg_nt_cnt < 100) ? PCT95TH: PCT90TH);
                        
            for (j = 0; j < an_dir->data[ix[i]].tot_nt_cnt; j++) { //for each nucleotide...
                
                //calculate a new normalized reactivity value
                if (isnan(an_dir->data[ix[i]].hq_profile[j])) {
                    
                    //if HQ reactivity is NaN set recalced value to NaN
                    an_dir->data[ix[i]].recalc_norm_profile[j] = NAN;
                } else {
                    //otherwise divide reactivity by current length normalization factor
                    an_dir->data[ix[i]].recalc_norm_profile[j] = an_dir->data[ix[i]].reactivity_profile[j]/cl_norm_factor;
                }
                ...
                if (!compare_float(an_dir->data[ix[i]].norm_profile[j], an_dir->data[ix[i]].recalc_norm_profile[j], 0.000001) && !isnan(an_dir->data[ix[i]].hq_profile[j])) {
                    test_failed = 1;
                }
            }
            ...
            if (verify_norm) {
                printf("trg %3d, cnf = %12.8f\t", ix[i], cl_norm_factor);
                if (test_failed) {
    ...
    for (i = 0; ix[i] <= an_dir->max_id; i++) {
        for (j = 0; j < an_dir->data[ix[i]].tot_nt_cnt; j++) {
            ...
            //option is on
            if (isHQnuc(&an_dir->data[ix[i]], j, min_depth, max_bkg) || norm_all) {
                an_dir->data[ix[i]].dataset_norm_profile[j] = an_dir->data[ix[i]].reactivity_profile[j]/an_dir->cnf;
            } else {
                an_dir->data[ix[i]].dataset_norm_profile[j] = NAN;
            }
            
            //set all dataset_norm_stderr values to NAN, since it is not calculated
            an_dir->data[ix[i]].dataset_norm_stderr[j] = NAN;
        }
    }
