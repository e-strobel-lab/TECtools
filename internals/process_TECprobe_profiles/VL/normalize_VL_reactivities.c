...
void normalize_VL_reactivities(SM2_analysis_directory * an_dir, int min_depth, double max_bkg, int verify)
{
    ...
    for (i = an_dir->min_tl, tot_rct_pf = 0; i <= an_dir->max_tl; i++) {
        ...
        //in verify mode, reset variables for recalculating individual normalization factors
        if (verify) {
        ...
        for (j = 0; j < an_dir->data[i].tot_nt_cnt; j++) {
            ...
            if (isHQnuc(&an_dir->data[i], j, min_depth, max_bkg)) {
                ...
                //in verify mode, add pf reactivity to list for the current transcript len
                if (verify) {
        ...
        //in verify mode, re-normalize all reactivity values and compare to the
        ...
        if (verify) {
            ...
            if (debug) {
                ...
                if (test_failed) {
                    printf("recalculated norm profiles are discordant\n");
                } else {
                    printf("full match\n");
                }
                
            //otherwise, print error message upon failure to recalculate correct reactivity value
            } else if (test_failed) {
                printf("normalize_VL_reactivities: error - in analysis directory %s, reactivity value(s) for transcript length %d were not recalculated correctly. please contact estrobel@buffalo.edu. aborting...\n", an_dir->prnt_dir_nm, an_dir->data[i].trg_nt_cnt);
                abort();
            }
        }
    }
